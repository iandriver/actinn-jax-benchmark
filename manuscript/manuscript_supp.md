...
*Supplement to "Cost, not accuracy, distinguishes cell-type annotation methods for
single-cell data". Figures and tables are referenced from the main text as Supplementary
Figure S1 to S13 and Supplementary Table S1 to S3.*
...
Cell Ontology, so an error under exact match and credit under ontology-aware concordance. The
...
at the wrong depth: hepatocyte called midzonal or centrilobular hepatocyte, NK cell called
hepatic pit cell, which is the Cell Ontology term for a liver NK cell. The focused pass scores
0.63 exact and recovers 0.05, because it is already right most of the time, so the ontology has
...
| SVM | [Pedregosa 2011] | classical | linear SVM (SGD) | none | scikit-learn |
| kNN | [Pedregosa 2011] | classical | k-nearest neighbors | none | scikit-learn |
...
| **scTOP** | [Yampolskaya 2023] | parameter-free | rank z-score class-average projection | none | NumPy |
| SingleR | [Aran 2019] | correlation | Spearman + fine-tuning | none | R |
...
| scPRINT | [Kalfon 2025] | foundation | pretrained transformer, zero-shot | none | PyTorch |
...
| lung_cross | [Sikkema 2023] to [Travaglini 2020] | cross-dataset | lung (HLCA to Krasnow) | 46 | Ensembl | different lab/protocol |
...
scale on three tissues. *A:* lung, 3.1k to 49k reference cells. *B:* the HLiCA liver atlas,
2.7k to 47k. *C:* Allen brain (MTG), 1.8k to 46k. In each one a prototype VAE moves from worst to
...
where actinn-jax rises 3.1 times from 5 to 86 types against the tuned linear pipeline's 11.0. SVM
...
10% for the linear pipeline, narrowing the advantage from 2.4 times to 2.1. *C:* peak memory,
which does not distinguish them, since on this axis it measures holding the query rather than
running the method. Three runs per point on a shared laptop: *A* and *B* report the fastest run, since
...
five that work share a shape, with accuracy and novelty rising as coverage falls, and the three
...
![accuracy against cost on every split](/Users/iandriver/Downloads/actinn-jax-benchmark/docs/figures/fig_pareto_facets.png)
...
end (pbmc 0.87 to 0.94, cross-dataset lung 0.33 to 0.38†), which is why Figure 1 normalizes. Most
