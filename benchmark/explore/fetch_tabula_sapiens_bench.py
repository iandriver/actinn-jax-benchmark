"""Pull a Tabula Sapiens multi-organ benchmark dataset (the LLM-leaderboard standard):
cell_type + CL ids + Ensembl var, raw counts in .X. Capped per type via within-dataset
joinid subsample. Run in .venv-scprint.
"""
...
PER_TYPE, CAP = 80, 16000
OUT = "/Volumes/IanSSD/tabula_sapiens_bench.h5ad"
...
with cellxgene_census.open_soma(census_version="stable") as census:
    ...
    print(f"TS obs: {len(obs):,} cells, {obs.cell_type.nunique()} types ({time.time()-t:.0f}s)", flush=True)
...
a.write_h5ad(OUT)
print(f"TS_BENCH_DONE {a.shape} {a.obs.cell_type.nunique()}", flush=True)
