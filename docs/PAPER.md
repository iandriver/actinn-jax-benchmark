...
projection (scTOP), marker/correlation (SingleR, scmap), the original TensorFlow ACTINN,
...
**The headline result is not that our method wins.** On the original ten-method panel
actinn-jax ties the top accuracy cluster (mean 0.831 vs. scANVI 0.835, a five-way cluster
within 0.004) while predicting ~195× faster than scANVI, and it places 3rd of 17 on the
community **Open Problems** `label_projection` benchmark (1st among methods completing every
dataset). But adding the strongest simple baselines from the concurrent literature changes
the picture. A carefully preprocessed **linear pipeline** (normalize → ANOVA → PCA →
logistic regression) **beats actinn-jax on both accuracy and training time** (mean
0.880/0.860 vs. 0.867/0.839 across four datasets, fitting 3.7× faster); and at atlas scale
**ProtoCloud** is the decisive accuracy winner, rising from worst at 3k reference cells
(0.722) to best at 49k (**0.976** vs. 0.936 for actinn-jax and 0.939 for the linear
pipeline). **actinn-jax is not the accuracy leader at any scale we tested**, and our earlier
margins over the "classical tier" reflected *untuned* baselines.
...
a withheld pulmonary ionocyte population and its marker ASCL3. actinn-jax is the **lightest
accurate method** in the panel (2.1× lower peak memory than the linear pipeline at 49k cells,
6.1 vs. 13.2 GB, with sub-second memory-bounded inference), which is what makes these
workflows practical on a laptop. We are explicit about what is *not* unique: ProtoCloud also
...
> **Within this ten-method panel** actinn-jax is Pareto-non-dominated: of the methods that
> match its accuracy none matches its speed, and of those matching its speed (SVM, kNN)
> none matches its accuracy. **That property does not survive the wider panel.** The tuned
> linear pipeline added later ([`SIMPLE_BASELINES.md`](SIMPLE_BASELINES.md)) is both more
> accurate (mean 0.880 vs 0.867) *and* faster to fit (3.7×), so it dominates actinn-jax on
> both axes; actinn-jax remains ~2× lighter in peak memory. We leave the original panel
> numbers as measured and flag the dominance here rather than restating a Pareto claim the
> later data refutes.
...
*within the original ten*, and [`SIMPLE_BASELINES.md`](SIMPLE_BASELINES.md) /
[`SCALING_MEMORY.md`](SCALING_MEMORY.md) for the current one.
