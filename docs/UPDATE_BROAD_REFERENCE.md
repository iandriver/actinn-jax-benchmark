...
`actinn_jax.bundled_reference("broad_human_v1")` is a pretrained census-wide model of 798
cell types in 28 coarse groups, about 50 MB, archived at
[doi:10.5281/zenodo.21688151](https://doi.org/10.5281/zenodo.21688151) and downloaded on
first use. This is how it gets rebuilt.

One step beyond the stages below: after a rebuild, re-archive it with
...
the CELLxGENE census, taking all primary human cells, stratified by `cell_type` at `PER_TYPE=40`
...
(`medium-v1.5`) over the reference in 4,000-cell chunks. scPRINT's QC drops cells, so the
embedding is not positionally aligned to the input. The label is carried through the model
as `carry_label` and saved beside the vectors, and stage 3 relies on that pairing.
...
HVG panel. Before shipping it runs the abstain calibration: 10% of cell types are withheld entirely as
out-of-distribution, plus a within-type test split, and `min_prob` is swept to trade
coverage against accuracy. That table is what the package README quotes.
...
Exact match is low, as expected: the census vocabulary and an atlas's vocabulary name the
same cell differently, calling a `periportal region hepatocyte` a `Hepatocyte`. Ontology
...
- The stages did not agree on filenames. Stage 2's default output (`/tmp/broad_emb.npz`) was
  not what stage 3 read (`/tmp/census_wide_emb.npz`), so a hand-run pipeline could train
  against a stale embedding with no error anywhere. Every path now derives from
  `$ACTINN_REF_WORK`.
- `stable` is a moving pointer, so the same command a month apart builds a different
  reference. Stage 1 records the resolved release to `census_release.json` and stage 3 copies
  it into the shipped `build_info.json`, so a reference can say what it came from. Pin
  explicitly with `CENSUS_VERSION=2025-11-08` for a reproducible rebuild.
- A rebuild can train cleanly and annotate worse. Stage 3 backs the previous reference up to
  `<name>.bak.<timestamp>` before overwriting, and stage 4 exists to catch this. Restore by
  moving the backup back over the reference directory.
...
| `PER_TYPE` | `40` | cells per cell type, driving both breadth and total size |
| `N_HVG` | `4000` | genes in the trained panel |
| `REF_NAME` | `broad_human_v1` | reference directory name, set to build a variant side by side |
...
That still wants an embedding for the hierarchy. To avoid the GPU entirely, use distillation
from a pretrained teacher: [PANHUMAN_DISTILL.md](PANHUMAN_DISTILL.md) builds a reference
whose hierarchy comes from Pan-human Azimuth's own levels, with no foundation model in the
loop. (`examples/build_reference.py` in the package discovers the hierarchy from scPRINT, so
it still needs one.)
